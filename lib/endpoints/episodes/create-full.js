    ...
    return Season.findById(seasonId)
      ...
      .then((nC) => {
        ...
        activeQueens.forEach((queen) => {
          newChallenges.forEach((challenge, i) => {
            promises.push(queen.addChallenge(challenge.id, {
      ...
      .then((nL) => {
        ...
        newLipsyncs.forEach((lipsync, i) => {
          req.body.lipsyncs[i].queens.forEach((queen) => {
            promiseArray.push(QueensLipsyncs.create({
