  ...
  db.transaction((transaction) => {
    return Season.findById(seasonId)
      ...
      .then((nL) => {
        ...
        return Promise.all([
          ...newLipsyncs.map((lipsync, i) => {
            return req.body.lipsyncs[i].queens.map(queenId => lipsync.addQueen(queenId, {
      ...
      .then(() => Promise.all(req.body.newJudges.map((j) => Judge.create({
        name: j.name,
      ...
      .then((eJ) => {
        return Promise.all([
          ...newJudges.map((judge) => judge.addEpisode(newEpisode.id)),
          ...ej.map((judge) => judge.addEpisode(newEpisode.id)),
        ]);
      })
      .catch((err) => {
        console.log(err);
