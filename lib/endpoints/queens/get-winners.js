...
function getWinners(req, res) {
  ...
  return Season.findAll({
    include: [
      { model: Queen, through: { where: { place: 1 } } }
    ]
  }).then(seasons => {
    seasons.forEach(season => {
      season.Queens.forEach(queen => {
        if (queen.id) {
          winnerIds = [...winnerIds, queen.id];
        }
      });
    });
  })
  .then(() => {
      return Promise.all(winnerIds.map(id => Queen.findByPk(id, { include: { model: Season } })));
  })
  .then(queens => {
